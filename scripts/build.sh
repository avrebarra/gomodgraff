...
go build "-ldflags=$GO_LDFLAGS" "-gcflags=$TRIM" "-asmflags=$TRIM" "$@" -o dist/gomodgraff cmd/gomodgraff/*

echo "(2/2) Packing up..."
pushd dist > /dev/null && tar -czf gomodgraff.tar.gz gomodgraff && popd > /dev/null
